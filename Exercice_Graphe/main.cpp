...
bool suiteSommets(s_reseau r, s_sommet* tabs);

void visiteS(s_reseau r, s_sommet s1, s_sommet s2);
...
int main() {
	...
	suiteSommets(Reseau, Reseau.tab_sommet);

	//25.1 
	//parcours profondeur récursif
	s.n_v = 0;
	visiteS(Reseau, s);// s est le sommet de départ
...
bool connectionCable(s_reseau Reseau, s_sommet s1, s_sommet s2) {
	...
	for (int i = 0; i < Reseau.n ; i++)
	{
		if (Reseau.tab_sommet[i].n_id == s1.n_id)
		{
...
bool graphNonOriente(s_reseau Reseau, s_sommet s1, s_sommet s2)
{
	...
	{
		if (connectionCable(Reseau, s2, s1) == true)
		{
			return true;
		}
		...
		/*for (int i = 0; i < Reseau.n; i++)
		{
			if (Reseau.tab_sommet[i].n_id == s2.n_id)
			{
				for (int j = 0; j < Reseau.n; j++)
				{
...
bool suiteSommets(s_reseau r, s_sommet* tabs) {
	...
	//boucle tant que , tant qu'il y a des sommet, mais surtout tant que ces sommet sont liés
	bool ok = true;
	bool result;
	int i = 0;
	...
	while (ok == true)
	{
		result = connectionCable(r, tabs[i], tabs[i + 1]);
		ok = result;
		i++;
...
void visiteS(s_reseau r, s_sommet s1, s_sommet s2)
{
	...
	for (int i = 0; i < r.n; i++)
	{
		...
		if (r.tab_sommet[i].n_id == s1.n_id)
		{

			for (int j = 0; j < r.n; j++)
			{

				if (r.tab_sommet[j].n_id == s2.n_id)
				{

					if (r.tab_matrice[i][j] == 1)
					{
						if (s2.n_v == 0)
						{
							s1 = s2;
							return visiteS(r, s1, s2);
						}
